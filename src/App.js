...
class App extends Component {
  ...
  onTabIdChanged = tabId => this.setState({category: tabId})

  isMatchedImage = imageUrl => {
    const {matchImg} = this.state

    if (matchImg === imageUrl) {
      const randomImageUrl = Math.floor(Math.random() * imagesList.length)
      console.log(randomImageUrl)
      this.setState(preState => ({
        score: preState.score + 1,
        matchImg: imagesList[randomImageUrl].imageUrl,
      }))
    } else {
  ...
  render() {
    const {category, matchImg, score, time} = this.state
    const categoryWise = imagesList.filter(each => each.category === category)
    const GameOvered = this.isGameOver()
    return (
      <div className="app-container">
        <nav className="nav-container">
          ...
          <ul className="score-and-timer-container">
            ...
            <li className="span-text"> {score}</li>
            <li>
              <img
                className="timmer-logo"
              ...
              <ul className="list-container">
                {tabsList.map(eachTab => (
                  <TabsList
                    eachTab={eachTab}
                    key={eachTab.tabId}
                    onTabIdChanged={this.onTabIdChanged}
                    activeButton={category === eachTab.tabId}
                  />
                ))}
              </ul>
            </div>
            <ul className="image-list-container">
              {categoryWise.map(eachImageList => (
                <ImageList
                  eachImageList={eachImageList}
                  key={eachImageList.id}
                  isMatchedImage={this.isMatchedImage}
                />
              ))}
            </ul>
          </div>
